...
| 분류 | 설명 | 장점 | 단점 |
| --- | --- | --- | --- |
| 선점형 | CPU를 강제로 뺏을 수 있는 방식. 프로세스가 중단될 수 있음. | - 자원 독점 방지
- 모든 프로세스에 골고루 자원을 배분 | - 문맥 교환에 오버헤드 발생
- 비효율적인 자원 분배 가능 |
| 비선점형 | CPU를 할당받은 프로세스가 종료될 때까지 다른 프로세스가 개입할 수 없음. | - 문맥 교환 오버헤드 적음 | - 자원 독점 가능
- 긴 프로세스가 시스템을 지배할 수 있음 |
